...
class Client:
    ...
    def __init__(
        ...
        *,
        ...
        # Remove args no longer supported by modern httpx
        if "proxies" in self.httpx_args:
            self.httpx_args.pop("proxies", None)

        # If httpx_args includes a timeout override, prefer it
        if "timeout" in self.httpx_args and not isinstance(self.httpx_args["timeout"], Timeout):
            self.httpx_args["timeout"] = Timeout(self.httpx_args["timeout"]) 
        if "timeout" in self.httpx_args:
            self.timeout = self.httpx_args["timeout"]
            # Avoid passing duplicate timeout kwarg
            self.httpx_args.pop("timeout", None)
    ...
    def get_httpx_client(self) -> httpx.Client:
        if self._client is None:
            ...
            # httpx.Client signature evolved; pass supported kwargs only.
            if self.timeout is not None:
                client_kwargs["timeout"] = self.timeout
            client_kwargs.update(self.httpx_args)
    ...
    def get_async_httpx_client(self) -> httpx.AsyncClient:
        if self._async_client is None:
            ...
            if self.timeout is not None:
                async_kwargs["timeout"] = self.timeout
            async_kwargs.update(self.httpx_args)
    ...
    async def _make_request_with_retries(
        ...
        last_exception = None
        full_url = f"{self.base_url.rstrip('/')}" + (path if path.startswith('/') else f"/{path}")
        ...
        for attempt in range(self.retry_config.max_retries + 1):
            ...
            try:
                ...
                # Record metrics
                if self.metrics:
                    self.metrics.record_request(200 <= response.status_code < 300, response_time)
